...
操作は、端末の名前の変更、原点の作り直し、表示の切り替えに留める。

表示する値は、受け手へ送っている値と同じものにする。
姿勢は REP-103 へ変換したあとの位置と向き、IMU は比力（m/s²）と角速度（rad/s）で出す。
ARKit や Core Motion の生の値を出すと、画面で見た軸の向きと受け手が受け取る軸の向きが食い違い、取り付けの確認に使えない。
...
| IMU | 比力、角速度、姿勢（度）、地磁気（µT）と較正の状態 |
| Environment | 気圧（hPa）、GNSS の測位と精度か許可の状態、電池、熱の状態、時計の自己点検 |
| Depth | タップで切り替える深度のヒートマップと、中心の距離 |

数値の更新は毎秒 5 回にする。
センサーの周期（100 Hz）で描き直すと、描画の負荷が配信を邪魔する。

IMU、気圧、電池は、購読が無くても画面の表示のために取得する。
消費電力は ARKit に比べて小さい。
画面の「Monitor」を切ると、購読されたセンサーだけを取得する動作になる。
GNSS は、購読されたときだけ取得する。
消費電力が大きく、最初の取得で位置情報の許可を求めるためである。
